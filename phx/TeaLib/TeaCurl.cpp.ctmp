...
void TeaCurl::setTimeout(long timeout) {
	curl_easy_setopt(this->ch, CURLOPT_TIMEOUT, timeout);
	curl_easy_setopt(this->ch, CURLOPT_CONNECTTIMEOUT, timeout);
}
...
CURLcode TeaCurl::getInfo(CURLINFO infoOpt, void *val) {
	return curl_easy_getinfo(this->ch, infoOpt, val);
}
